package com.quijano.gateway.beans;

import com.quijano.gateway.filters.AuthFilter;
...
public class GatewayBeans {
    ...
    public RouteLocator routeLocatorOauth2(RouteLocatorBuilder builder){
        return builder
                ...
                .route(route -> route
                        ...
                        .filters(filter ->{
                            filter.circuitBreaker(config -> config
                                    ...
                                    .setStatusCodes(Set.of("500", "400"))
                                    .setFallbackUri("forward:/companies-crud-fallback/company/*"));
                            filter.filter(this.authFilter); // Se agrega el filtro
                            return filter;
                        })
                        .uri("lb://companies-crud")
                ).
                route(route -> route
                        .path("/report-ms/report/**")
                        .filters(filter -> filter.filter(this.authFilter)) // Se agrega el filtro
                        .uri("lb://report-ms")
                )
                .route(route -> route
                        .path("/companies-crud-fallback/company/**")
